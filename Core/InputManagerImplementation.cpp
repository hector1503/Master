			...
			for (int i = 0; i < l_Input.GetNumChildren(); ++i)
			{
				...
				if (l_Element.GetName() == std::string("action"))
				{
					...
					case MOUSE:
						...
						button = l_Action.GetPszProperty("button", "RIGHT");
						action.mouse.button = ParseButton(button);
						
						break;

					case GAMEPAD:
						button = l_Action.GetPszProperty("button", "XINPUT_GAMEPAD_DPAD_RIGHT");
						action.gamepad.button = ParseGButton(button);
						action.gamepad.gamepadNumber = l_Action.GetIntProperty ("number", 0.05);
				...
				else if (l_Element.GetName() == std::string("axis"))
				{
					...
					Axis axis = {};
					axis.name = l_Axis.GetPszProperty("name")[0];
					...
					{
					case MOUSE:
						Maxis = l_Axis.GetPszProperty("axis", "X")[0];
						axis.mouse.axis = ParseMaxis(Maxis);
						axis.mouse.scale = l_Axis.GetFloatProperty ("scale", 0.05);
						break;
					case GAMEPAD:
						Gaxis = l_Axis.GetPszProperty("axis", "X")[0];
						axis.gamepad.axis = ParseGaxis(Gaxis);
						axis.gamepad.gamepadNumber = l_Axis.GetIntProperty ("number", 0.05);
						break;
...
//Action::MouseButton ParseButton(const std::string& button);
CInputManagerImplementation::Action::MouseButton CInputManagerImplementation::ParseButton(const std::string& button)
{
	if (button == "LEFT")
	{
		return Action::LEFT;
...
CInputManagerImplementation::MouseAxis CInputManagerImplementation::ParseMaxis(const std::string& maxis)
{
	if (maxis == "X")
	{
		return MouseAxis::X;
...
unsigned short CInputManagerImplementation::ParseGButton(const std::string& button)
{
...
CInputManagerImplementation::GamepadAxis CInputManagerImplementation::ParseGaxis(const std::string& maxis)
{
	if (maxis == "LEFT_X")
	{
		return GamepadAxis::LEFT_X;
	}
	else if (maxis == "LEFT_Y")
	{
	...
	else if (maxis == "RIGHT_Y")
	{
		return GamepadAxis::RIGHT_Y;
	}
	if (maxis == "LEFT_TRIGGER")
	{
		return GamepadAxis::LEFT_TRIGGER;
	}
	else if (maxis == "RIGHT_TRIGGER")
	{
	...
	if (m_Focus)
	{
		...
		for (std::vector<Action>::const_iterator it = m_Actions.cbegin(); it != m_Actions.cend(); ++it)
		{
			...
			case Action::ON_RELEASE:
				// TODONE: añadir acciones de release
				if (otherNeeds && !current && previous)
				{
			...
			case Action::WHILE_RELEASED:
				// TODONE: añadir acciones de botón no pulsado
				if (otherNeeds && !current)
				{
					isActionActive = false;
				}
		...
		for (std::vector<Axis>::const_iterator it = m_Axis.cbegin(); it != m_Axis.cend(); ++it)
		{
			...
			case GAMEPAD:
				...
				case GamepadAxis::RIGHT_TRIGGER:
					if (l_ControllerPresent[axis.gamepad.gamepadNumber])
					{
						BYTE rawState = l_ControllerState[axis.gamepad.gamepadNumber].Gamepad.bRightTrigger;
						if (rawState > XINPUT_GAMEPAD_TRIGGER_THRESHOLD)
						{
							m_ActiveAxis[axis.name] += (float)((rawState + XINPUT_GAMEPAD_TRIGGER_THRESHOLD) / (256.0f - XINPUT_GAMEPAD_TRIGGER_THRESHOLD));
						}
